...
/* ½«³¤¶ÈÎªlenµÄ×Ö·û´®´Ósrc¸´ÖÆµ½dest´¦£¬Õâ¸ö¹ý³ÌÖÐ
  * »á¸ø\ ¼ÓÉÏ\±ä³É\\£¬¸ø"¼ÓÉÏ\±ä³É\"
  * ²»¿ÉÏÔÊ¾µÄ×Ö·û½«Æä×ª»»³É8½øÖÆ£¬½«ÆäÊýÖµ×Ö·û´®´æÈëdest
  * ·µ»ØÖµÊÇÄ¿±ê×Ö·û´®µÄÄ©Î²
  * Õâ¸öº¯ÊýµÄ¹¦ÄÜÊµ¼ÊÉÏÊÇ¸ø×Ö·û´®¼ÓÉÏË«ÒýºÅ
  */
U_CHAR *
...
unsigned int len;
{
	while (len--)
	{
		...
		{
			if (ISPRINT(c)) //ÅÐ¶ÏÊÇ·ñÊÇ¿ÉÏÔÊ¾µÄ×Ö·û
				*dest++ = c;
			else
			{
...
/* Ê¹ÓÃ±ê×¼CµÄ#-²Ù×÷·û¹æÔò½«Ò»ÏµÁÐµÄ²ÎÊý×ª»¯ÎªÒ»¸ö×Ö·û´® */
static const cpp_token *
...
macro_arg *arg;
{
	...
	for (i = 0; i < arg->count; i++)
	{
		...
		/* Èç¹ûÊÇ×Ö·û´®ÀàÐÍµÄtoken£¬½«±êÖ¾ÖÃÎªÕæ */
		escape_it = (token->type == CPP_STRING || token->type == CPP_WSTRING
			|| token->type == CPP_CHAR || token->type == CPP_WCHAR);

		/* Room for each char being written in octal, initial space and
		final NUL.  */
		/* Îª½«×Ö·û×ª»»³ÉÓÃ×ªÒå×Ö·û´®±í´ïµÄ°Ë½øÖÆÊý×¼±¸¿Õ¼ä */
		len = cpp_token_len(token);
		if (escape_it)
			len *= 4;
		len += 2;

		/* Èç¹ûÒòÎªÐèÒª¸ü¶àµÄ¿Õ¼äÊ¹Ô­À´µÄ»º³å²»¹»ÓÃ£¬ÔòÒªÔö¼Ó»º³å */
		...
		if (escape_it)
			/* Èç¹ûÊÇ×Ö·ûÐÍ»ò×Ö·û´®ÐÍµÄtoken £¬
			  * ½«Æä×ª»¯³É×Ö·û´®(´øË«ÒýºÅµÄ×Ö·û´®)
			  */
		{
	...
	/* Èç¹û×îºóÁ¬ÐøµÄ·´Ð±¸ÜµÄ¸öÊýÊÇÆæÊý */
...
/* ³¢ÊÔÕ³ÌùÁ½¸ötoken£¬¼´½«Á½¸ötoken×ª»»³É×Ö·û´®ºóÁ¬½ÓÆðÀ´£¬
  * »áÉ¨Ãè½øÒ»¸öÐÂµÄtoken£¬Õâ¸ötokenÊÇÓÃÀ´Ìæ»»Ç°Ò»¸ötokenµÄ¡£
  * ¼´ÓÒÌæ»»×ó¡£
  */
static bool
...
const cpp_token **plhs, *rhs;
{
	...
	lhs = *plhs;
	len = cpp_token_len(lhs) + cpp_token_len(rhs) + 1;  //Á½¸ötoken³¤¶ÈµÄºÍ¼Ó1
	buf = (unsigned char *)alloca(len);                 //ÉêÇë³¤¶ÈÎªlenµÄ¿Õ¼ä
	end = cpp_spell_token(pfile, lhs, buf);             //½«×ótoken×ª»»Îª×Ö·û±íÊ¾

	/* Avoid comment headers, since they are still processed in stage 3.
	It is simpler to insert a space here, rather than modifying the
	lexer to ignore comments in some circumstances.  Simply returning
	false doesn't work, since we want to clear the PASTE_LEFT flag.  */
	/* ±ÜÃâ×¢ÊÍµôÍ·ÎÄ¼þ£¬ÒòÎªËûÃÇÔÚµÚ3½×¶ÎÈÔÔÚ´¦ÀíÖÐ¡£ 
	ÔÚÕâÀï²åÈëÒ»¸ö¿Õ¸ñ±ÈÐÞ¸Ä´Ê·¨·ÖÎöÆ÷À´ºöÂÔÔÚ
	Ä³Ð©Çé¿öÏÂµÄ×¢ÊÍÒª¼òµ¥¡£¼òµ¥µØ·µ»Ø ¼ÙµÄÒ²²»ÐÐ£¬
	ÒòÎªÎÒÃÇÒªÇå³ýPASTE_LEFT±êÖ¾¡£ */
	/* Èç¹û×ó²Ù×÷·ûµÄÀàÐÍÊÇ³ýºÅ²¢ÇÒÓÒ²Ù×÷·ûµÄÀàÐÍÊÇ³ËºÅ»òÕß³ýºÅ 
	  * ¼´Èç¹û×óÓÒtoken¹¹³É//»òÕß/*
	  */
	if (lhs->type == CPP_DIV
		&& (rhs->type == CPP_MULT || rhs->type == CPP_DIV))
		*end++ = ' ';	//ÆÆ»µµô×¢ÊÍ
	/* ½«ÓÒ²Ù×÷·û×ª»»³É×Ö·û´®ºÍ×ó²Ù×÷·ûµÄ×Ö·û´®Á¬ÆðÀ´ */
	end = cpp_spell_token(pfile, rhs, end);
	*end = '\0';    //Ìí¼Ó½áÊø·û

	/* ½«Íê³ÉµÄ×Ö·û´®±£´æµ½Õ»ÖÐ£¬stage3 */
	...
	/* ²»ÏëÈÃ¿ªÍ·µÄ#±»ÈÏÎªÊÇÖ¸ÁîµÄ¿ªÊ¼ */
	pfile->buffer->saved_flags = 0;

	/* Set pfile->cur_token as required by _cpp_lex_direct.  */
	/* »ñÈ¡Ò»¸öºÍÇ°Ò»¸ötokenÓÐÏàÍ¬Î»ÖÃÐÅÏ¢µÄtoken£¬
	  * ÕâÑù»ñÈ¡µÄÏÂÒ»¸ötoken¾ÍºÍÇ°Ò»¸ötokenÔÚÏàÍ¬Î»ÖÃÉÏÁË
	  */
...
const cpp_token *lhs;
{
	...
	do
	{
		...
		/* Ö±½Ó´Óµ±Ç°ÉÏÏÂÎÄ»ñÈ¡token£¬ÎÒÃÇ¿ÉÒÔ×ö 
			µ½ÕâÒ»µã£¬ÒòÎªÎÒÃÇÔÚobject-likeºêµÄÌæ»»ÁÐ±í 
			»ò²åÈë²ÎÊýµÄº¯Êýºê 
			£¬ÔÚÕâÁ½ÖÖÇé¿öÏÂ£¬Ô¼Êø#define 
			±£Ö¤ÖÁÉÙÓÐÒ»¸öÒÔÉÏµÄÁîÅÆ¡£ */
		if (context->direct_p)
	...
	} while (rhs->flags & PASTE_LEFT);  //¼ÌÐøÌæ»»ÎªÌæ»»²¿·Ö

	/* Put the resulting token in its own context.  */
	/* ½«token±£´æµ½Ëü×Ô¼ºµÄcontextÀïÃæ */
...
const cpp_hashnode *node;
{
	...
	/* ËÑ¼¯×é³É²ÎÊýµÄtoken¡£ÏÖÔÚ»¹²»ÄÜÈ·¶¨ÓÐ¶àÉÙ¸ö²ÎÊý¡£
	  * 
	  */
	do
	{
		...
		for (;;)
		{
			...
			/* ÒªÁôÓÐÁ½¸ötokenµÄ¿Õ¼ä¸øÐÂtokenÓÃ */
			...
			if (token->type == CPP_PADDING)
			{
				...
				/* ¶ªÆú¿ªÍ·µÄpadding token */
				if (ntokens == 0)
					continue;
			}
			else if (token->type == CPP_OPEN_PAREN)
				/* ¼ÇÂ¼×óÀ¨ºÅºÍÓÒÀ¨ºÅµÄ²îÖµ */
				paren_depth++;
			else if (token->type == CPP_CLOSE_PAREN)
			{
				/* ÓÒÀ¨ºÅ±È×óÀ¨ºÅ¶à£¬´íÎó */
				if (paren_depth-- == 0)
					break;
			}
			else if (token->type == CPP_COMMA)
			{
				...
				/* ÕýÈ·µØËÑ¼¯µ½ËùÓÐµÄ²ÎÊý */
				if (paren_depth == 0
					&& !(macro->variadic && argc == macro->paramc))
					break;
			}
			/* »»ÐÐÁË£¬»òÕß¿ªÍ·µÚÒ»¸ötokenÊÇ# */
	...
	/* ½áÊøµÄtokenÊÇCPP_EOF »òÕßCPP_HASHÀàÐÍµÄ»° */
	if (token->type == CPP_EOF || token->type == CPP_HASH)
	{
		...
		/* Èç¹ûµ±Ç°context»¹ÓÐÇ°Ò»¸öcontext»òÕßÏÖÔÚµÄ×´Ì¬ÊÇÔÚÖ¸ÁîÀï */
		else
	...
	/* Èç¹û½áÊøµÄtokenÊÇCPP_CLOSE_PAREN ²¢ÇÒ»¹Ã»ÓÐ´¦ÀíËùÓÐµÄ²ÎÊý */
...
/*
  *Ñ°ÕÒÒ»¸ö½ÚµãµÄºê¿ªÊ¼µÄ×óÀ¨ºÅ£¬ÔÚÕâÑùµÄ 
»°£¬Èç¹ûÃ»ÓÐÕÒµ½£¬ÎÒÃÇ²»»á¶ªÊ§ÒÔÈÎºÎ·½Ê½ 
ÖÐ¼äÌî³äµÄ±ê¼ÇÐÅÏ¢¡£Èç¹ûÎÒÃÇÕÒµ½ÁËÀ¨ºÅ£¬ÊÕ¼¯ 
²ÎÊý²¢·µ»Ø°üº¬ËüÃÇµÄ»º³åÇø¡£
  *
  *
  *
  *
...
cpp_hashnode *node;
{
	...
	for (;;)
	{
		/* Óöµ½²»ÊÇCPP_PADDINGÀàÐÍµÄtoken¾ÍÖÐ¶ÏÑ­»· */
		token = cpp_get_token(pfile);
		if (token->type != CPP_PADDING)
			break;
		if (padding == NULL
	...
	if (token->type != CPP_EOF || token == &pfile->eof)
	{
		...
		/* ±£´æÒ»¸ötoken */
		_cpp_backup_tokens(pfile, 1);
		/* Èç¹ûÌø¹ýÁËpadding token£¬½«Æä±£´æÏÂÀ´ */
		if (padding)
...
/*
  * Èç¹û³É¹¦Õ¹¿ªºê£¬Ôò½«Æä¿ÉÌæ»»ÁÐ±íÑ¹ÈëÉÏÏÂÎÄ¶ÑÕ»£¬·µ»Ø1
  * Ê§°ÜµÄ»°·µ»ØÁã¡£
  *
  *
  *
...
cpp_hashnode *node;
{
		...
		/* ²»ÊÇÄÚÇ¶µÄºê */
	{
			...
			/* Èç¹ûÊÇº¯ÊýÀàËÆºê */
		{
			...
			/* Ñ°ÕÒÒÔ×óÀ¨ºÅ¿ªÊ¼µÄºêµÄ²ÎÊýÁÐ±í */
...
macro_arg *args;
{
	...
	/* ¶ÔÃ¿¸öºê±äÁ¿½øÐÐ¼ì²é£¬Èç¹û±äÁ¿ÊÇºêµÄ»°½øÐÐÏàÓ¦´¦Àí */
	for (src = macro->expansion; src < limit; src++)
	if (src->type == CPP_MACRO_ARG)
	{
		/* Leading and trailing padding tokens.  */
		/* ¼ÓÉÏ¿ªÍ·ºÍ½áÎ²µÄÁ½¸ötoken */
		total += 2;

		/* We have an argument.  If it is not being stringified or
		pasted it is macro-replaced before insertion.  */
		/* Èç¹ûµ±Ç°µÄtokenÃ»ÓÐ×Ö·û´®»¯Ò²Ã»ÓÐ½øÐÐÕ³ÌùÀ©Õ¹£¬
		  * ¾ÍÐèÒª¶ÔÆä×öºêÕ¹¿ª
		  */
	...
	the arguments.  */
	/* µ½´ËÎªÖ¹ÒÑ¾­½«tokenµÄÐÎÊ½×ª»»ÎªÈÝÒ×´¦ÀíµÄÐÎÊ½ÁË£¬
	  * ÏÖÔÚ·ÖÅä×ã¹»µÄ¿Õ¼ä£¬ÓÃtokenÌæ»»µô²ÎÊý
	  *
	  */
	...
	for (src = macro->expansion; src < limit; src++)
	{
		...
		arg = &args[src->val.arg_no - 1];
		if (src->flags & STRINGIFY_ARG)
			/* Èç¹ûÊÇ±»×Ö·û´®»¯¹ýµÄtoken£¬»ñÈ¡Æä×Ö·û´®»¯ºóµÄÖ¸Õë */
		{
			count = 1, from = &arg->stringified;
		}
		else if (src->flags & PASTE_LEFT)
			/* Èç¹ûÊÇÌù×óÐÍµÄtoken£¬»ñÈ¡Ö¸Õë */
		{
			count = arg->count, from = arg->first;
		}
		else if (src != macro->expansion && (src[-1].flags & PASTE_LEFT))
			/* Èç¹ûsrc²»ÊÇÁÐ±íµÄµÚÒ»¸ö£¬²¢ÇÒsrcÇ°Ò»¸ötokenÊÇÌù×óÀàÐÍ */
		{
			count = arg->count, from = arg->first;
			/* Èç¹ûÕâ²»ÊÇµÚÒ»¸ötoken */
			if (dest != first)
			{
				...
				/* Èç¹ûÓÒ²Ù×÷·ûÊÇÒ»¸öÎ»ÖÃ±ê¼Ç£¬Ôò½«paste±êÖ¾ÒÆ³ý */
				else if (count == 0)
					paste_flag = dest - 1;
			}
		}
		...
		/* ÔÚ²ÎÊýµÄ×ó±ß¼ÓÉÏÌî³ätoken£¬³ý·ÇÊÇ##µÄÓÒ²Ù×÷·û */
		if (!pfile->state.in_directive
			&& src != macro->expansion && !(src[-1].flags & PASTE_LEFT))
			...
			/* ÓÐ²ÎÊýÐèÒª¸´ÖÆ */
		{
			...
			/* ## µÄ×ó²Ù×÷²ÎÊýÊÇ·Ç¿ÕµÄ£¬Ç°Ò»¸ötoken¾ÍÓ¦¸ÃÊÇ±»±ê¼ÇÎªPASTED_LEFT */
			if (src->flags & PASTE_LEFT)
				paste_flag = dest - 1;
		}

		/* Avoid paste on RHS (even case count == 0).  */
		/* ±ÜÃâÕ³ÌùÓÒ²Ù×÷²ÎÊý¼´Ê¹countÎªÁã */
		if (!pfile->state.in_directive && !(src->flags & PASTE_LEFT))
			*dest++ = &pfile->avoid_paste;

		/* Add a new paste flag, or remove an unwanted one.  */
		/* Ôö¼ÓÒ»¸öÐÂµÄ±êÖ¾»òÕßÒÆ³ýÃ»ÓÃµÄ±êÖ¾ */
		if (paste_flag)
		{
			/* »ñÈ¡Ò»¸öÓ°×Ótoken */
			cpp_token *token = _cpp_temp_token(pfile);
	...
	/* ½«À©Õ¹ºÃµÄtokenÁ´±íÑ¹ÈëÕ» */
	push_ptoken_context(pfile, node, buff, first, dest - first);
}

/* Return a special padding token, with padding inherited from SOURCE.  */
/* ·µ»ØÒ»¸öÌî³ätoken£¬´Ósource¼Ì³ÐÐÅÏ¢ */
static const cpp_token *
padding_token(pfile, source)
cpp_reader *pfile;
const cpp_token *source;
{
	/* »ñÈ¡Ò»¸öºÍµ±Ç°tokenÓÐÏàÍ¬ÐÐÁÐºÅµÄtoken */
...
/* ½«firstµ½first+countµÄtoken±£´æµ½ÏÂÒ»¸öcontextÀï£¬pfileµÄcontextÒ²±ä³ÉÕâ¸öcontext */
static void
...
/*ÔÚÌæ»»º¯ÊýÐÍºêµÄµÄ²ÎÊýÖ®Ç°Õ¹¿ªÒ»¸ö²ÎÊýarg ¡£
Í¨¹ý±£´æÉÏÏÂÎÄµÄ²ÎÊýµÄtoken£¬È»ºó¾ÍºÃÏñËüÊÇ¼ÇºÅÁ÷
µÄÕý³£×é³É²¿·ÖÒ»Ñù½«ÆäÀ©Õ¹µ½ÁÙÊ±»º³å¡£
collect_args()ÒÑÍ¨¹ýCPP_EOFÖÕÖ¹²ÎÊýµÄtokenÈÃÎÒÃÇÖªµÀ 
ÎÒÃÇÊ²Ã´Ê±ºòÍêÈ«Õ¹¿ªÁË²ÎÊý¡£
*/
static void
expand_arg(pfile, arg)
cpp_reader *pfile;
...
/* ÊÍ·Åµô»º³å£¬²¢Ê¹ÄÜºê¡£µ«context½á¹¹±£ÁôÏÂÀ´¡£ */
void
_cpp_pop_context(pfile)
cpp_reader *pfile;
{
...
pre-expansion.  */
/* »ñÈ¡Ò»¸ötoken¡£ÕâÀïÖ»ÓÐÔÚÎÄ¼þ½áÊø²Å»á»ñµÃCPP_EOF */
const cpp_token *
cpp_get_token(pfile)
cpp_reader *pfile;
{
	...
	for (;;)
	{
		...
		/* ÕâÊÇµÚÒ»¸öcontextµÄ»° */
		if (!context->prev)  //¶ÁÈ¡Ò»¸ötoken
			result = _cpp_lex_token(pfile);
		/* contextÀïÓÐ¿ÉÓÃtoken£¬¾Í´ÓcontextÀï»ñÈ¡ */
		...
		/* ÒªÊ¹ÓÃÇ°Ò»¸öcontextµÄtoken */
		else
		{
			/* ´ÓcontextÕ»ÖÐÈ¡³öÉÏÒ»¸öcontext */
			_cpp_pop_context(pfile);
			if (pfile->state.in_directive)
				continue;
		...
		/* lex µÃµ½ÐÂµÄtokenÊÇ±êÊ¶·ûÃû£¬OK·µ»Ø*/
		...
		if (!(node->flags & NODE_DISABLED))
		{
			if (!pfile->state.prevent_expansion
				&& enter_macro_context(pfile, node))
				/* Ã»ÓÐÉèÖÃ×éÖ¯Õ¹¿ªµÄ±êÖ¾ */
			{
				if (pfile->state.in_directive)
					continue;
				/* ·µ»ØÒ»¸ö¼Ì³ÐÏàËÆÎ»ÖÃÐÅÏ¢µÄtoken*/
				return padding_token(pfile, result);
			}
		}
		else
...
/* tokenÖ¸ÕëÏòºóÍË¼¸¸öÎ»ÖÃ¡£µ«ÊÇÖ»ÓÐÔÚlexerÀï²ÅÄÜÍË¶àÓÚ1²½ */
void
...
unsigned int count;
{
	if (pfile->context->prev == NULL)
	{
		...
		{
			pfile->cur_token--;
			/* ÍËµ½ÁËµ±Ç°tokenrunµÄµÚÒ»¸ötoken£¬ÔÙÍË¾ÍÊÇÇ°Ò»¸ötokenrunµÄÄ©Î²ÁË */
