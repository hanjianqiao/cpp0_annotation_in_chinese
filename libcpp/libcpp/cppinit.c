...
const char *input;
{
	...
	while (mx > mn)
	{
		...
		/* 这个函数是比较两个字符串的内存差
		  * 第三个参数指定比较的字符个数
		  * 如果在指定的字符个数内两个字符串字符相同，则返回0
		  * 否则返回前一个字符串与后一个字符串地址的差
		  */
		...
		{
			/* 可能会遇到搜索abcde遇到abc的情况
			  * 如果最后一个字符是结束符，则是符合
			  * 如果最后一个是普通字符，就是不同的字符串
			  */
			if (input[opt_len] == '\0')
				return md;
			...
			/* 这里取向前搜索的方式，应该是因为在构建列表时
			  *  按照习惯abcd是紧跟这abc的
			  */
			mn = md + 1;

			/* 如果搜索到的这个不完全选项有附加信息，
			  * 继续查找，返回最长匹配且带有附加信息的选项索引，
			  * 或者返回完全匹配的下标
			  */
...
int ignore;
{
	...
	/* 非选项的字符串都是文件名，这里虽然好像可以把"-"作为文件名，但实际上不行 */
	...
	else	/* argv[i][0] == '-' && argv[i][1] != '\0'，选项 */
	{
		...
		if (cl_options[opt_index].msg)
		{
			...
			/* 由于带而外信息的选项可能返回的是不完全匹配
			  * 所以arg[0]很可能不是'\0'
			  */
			if (arg[0] == '\0' && opt_code != OPT_W)
			{
				/* 无附加信息的选项，除了W外应该都要有选项参数 */
				arg = argv[++i];
				if (!arg)
				{
		...
		/* 处理选项
		  * 简单来说就是根据选项及参数设置一下pfile结构体一些成员的值
		  *
		  */
		switch (opt_code)
		{
