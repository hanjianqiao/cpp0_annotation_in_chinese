...
/* The dev_t and ino_t, sys / types.h need */
typedef int ino_t;
typedef int dev_t;
...
struct lexer_state
{
	...
	/* Ò»ÐÐÖÐµÚÒ»¸öTokenÊÇCPP_HASHµÄ»°£¬Õâ¸öÖµ¾Í²»ÊÇ0 */
...
/* ±£´æcpplib¶ÁÈEÄÎÄ¼þÄÚÈÝ */
struct cpp_buffer
{
	const unsigned char *cur;	 /* current position */ /* µ±Ç°Î»ÖÃ */
	const unsigned char *backup_to; /* if peeked character is not wanted *//* Èç¹û»ñÈ¡µÄ×Ö·ûÊÇÒâÍâµÄ */
	const unsigned char *rlimit; /* end of valid data *//* ÓÐÐ§Êý¾ÝµÄ½áÎ² */
	const unsigned char *line_base; /* start of current line *//* µ±Ç°ÐÐµÄ¿ªÍ· */
	...
	/* Ö¸ÏòÒýÓÃ±ú½£Èç¹ûÕâÊÇÎÄ¼þ»º³åµÄ»°£¬incÊÇ·ÇÁã¡£
	  * ÓÃÀ´¸¨Öúinclude_nextºÍ¼ÇÂ¼¿ØÖÆºê¡£
	  */
	struct include_file *inc;

	/* Value of if_stack at start of this file.
	Used to prohibit unmatched #endif (etc) in an include file.  */
	/* Õâ¸öÎÄ¼þµÄ¿ªÊ¼µÄif_stackµÄÖµ ¡£
	  * ÓÃÀ´½ûÖ¹Ò»¸öÒýÓÃÎÄ¼þÄÚÃ»ÓÐÆ¥ÅäµÄ#endifµÈ¡£
	  */
	struct if_stack *if_stack;

	/* Token column position adjustment owing to tabs in whitespace.  */
	/* ÓÉÓÚtabÒýÆðµÄtokenµÄÁÐµÄÎ»ÖÃµ÷ÕE*/
	...
	/* ÓÉÓÚ´Ê·¨·ÖÎöµÄ¹¤×÷Ô­Àú¿¬-Wtrigraphs¿ÉÄÜÔÚÓÐÐ©Ê±ºò»E
	  * Ê¹µÃÍ¬Ò»¸öÈýÄ¿Ê½·¢³öÁ½´Î¾¯¸æ¡£
	...
	be far too noisy).  */
	/* Èç¹ûÒÑ¾­¶ÔÕâ¸öÎÄ¼þÄÚµÄC++·ç¸ñ×¢ÊÍ·¢³ö¹ý¾¯¸æ£¬Õâ¸ö±äÁ¿ÎªÕæ¡£
	  * Ö»ÓÐ¶ÔÓÚC89À©Õ¹Ä£Ê½µÄ-pedanticÊ¹ÄÜ»òÕß-WtraditionalÊ±²Å»á·¢Éú£¬
	  * ²¢ÇÒÖ»»á·¢³öÒ»´Î¾¯¸æ¡£
	  */
	...
	buffers.  */
	/* µ±Ã»ÓÐÔÚ´¦ÀúäýÄ¿Ê½ºÍ×ªÒå»»ÐÐµÄÊ±ºòÕâ¸ö±äÁ¿ÎªÕæ¡£
	...
	/* ·ÇÁãÖµ±úæ¾¿ªÊ¼ËÑËE"ÀàÐÍÒýÓÃÎÄ¼þµÄÄ¿Â¼ÒÑ¾­
	  * ±»´¦ÀúÕý²¢´æ·ÅÔÚÏÂÃæµÄdir±äÁ¿ÖÐ¡£
	  */
	...
	/* »º³åÇøÔÚÎÄ¼þÄ©Î²»á×Ô¶¯µ¯³ö¡£
	  * Èç¹ûÕâ¸öÖµÎªÕæ£¬ÔÚÎÄ¼þÄ©Î²·µ»ØµÄÊÇCPP_EOF token£¬
