...
struct cpp_buffer
{
	const unsigned char *cur;	 /* current position */ /* 当前位置 */
	const unsigned char *backup_to; /* if peeked character is not wanted *//* 如果获取的字符是意外的 */
	const unsigned char *rlimit; /* end of valid data *//* 有效数据的结尾 */
	const unsigned char *line_base; /* start of current line *//* 当前行的开头 */
	...
	/* 指向引用柄剑如果这是文件缓冲的话，inc是非零。
	  * 用来辅助include_next和记录控制宏。
	  */
	...
	be far too noisy).  */
	/* 如果已经对这个文件内的C++风格注释发出过警告，这个变量为真。
	  * 只有对于C89扩展模式的-pedantic使能或者-Wtraditional时才会发生，
	  * 并且只会发出一次警告。
	  */
	...
	buffers.  */
	/* 当没有在处理三目式和转义换行的时候这个变量为真。
	 * 对于preprocessed input、command line directives和_Pragma buffers这个变量的值为真。
	 */
	unsigned char from_stage3;

	/* Nonzero means that the directory to start searching for ""
	include files has been calculated and stored in "dir" below.  */
	/* 非零值表示开始搜""类型引用文件的目录已经
	 * 被处理正并存放在下面的dir变量中。
	 */
	...
	/* 缓冲区在文件末尾会自动弹出。
	  * 如果这个值为真，在文件末尾返回的是CPP_EOF token，
	  * 否则的话，当前buffer临近的缓冲区的token会被返回。
	  */
	bool return_at_eof;
