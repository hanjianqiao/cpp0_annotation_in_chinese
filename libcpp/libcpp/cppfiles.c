...
/* ËùÓÐÎÄ¼þµÄ±íÊ¾ÓÃÕâ¸ö½á¹¹Ìå */
struct include_file
{
	const char *name;		/* actual path name of file *//* ÎÄ¼þµÄÕæÊµÂ·¾¶Ãû³Æ */
	const cpp_hashnode *cmacro;	/* macro, if any, preventing reinclusion.  *//* ÓÃÀ´·ÀÖ¹ºêµÄÖØ¸´ÒýÓÃ */
	const struct search_path *foundhere;/* ÔÚËÑË÷Â·¾¶ÖÐÕÒµ½ÎÄ¼þµÄÎ»ÖÃ£¬#include_nextºÍsysp¡£ */
	/* location in search path where file was
	found, for #include_next and sysp.  */
	const unsigned char *buffer;	/* pointer to cached file contents *//* Ö¸Ïò»º³åÁËµÄÎÄ¼þÄÚÈÝ */
	unsigned int size;/* ÎÄ¼þ´óÐ¡£¿ */
#if 0
	struct stat st;		/* copy of stat(2) data for file */
#endif
	FILE *fp;			/* fd open on file (short term storage only) *//* ÎÄ¼þÖ¸Õë */
	int err_no;			/* errno obtained if opening a file failed *//* ÎÄ¼þ´ò¿ªÊ§°ÜµÄ´íÎó´úÂë */
	unsigned short include_count;	/* number of times file has been read *//* ÎÄ¼þ±»¶ÁÈ¡µÄ´ÎÊý */
	unsigned short refcnt;	/* number of stacked buffers using this file *//* ÒýÓÃ¸ÃÎÄ¼þµÄÒÑÑ¹Õ»µÄ»º³å¸öÊý */
	unsigned char mapped;		/* file buffer is mmapped *//* ÎÄ¼þ»º³åÊÇ·ñmmapped */
};
...
const char *fname;
{
	...
	/* Èç¹ûÔÚÉìÕ¹Ê÷ÖÐµ½¸ÃÂ·¾¶£¬Ôò·µ»Ø£¬·ñÔò´´½¨Ò»¸ö½Úµã*/
	if (node)
		free(name);
	else
	{
...
enum include_type type;
{
	...
	if (type == IT_INCLUDE_NEXT && pfile->buffer->inc->foundhere)
		...
		path = search_from(pfile, type);  //»ñÈ¡includeÎÄ¼þµÄËÑË÷Ä¿Â¼
	...
	/* Search directory path for the file.  */
	/* ÔÚÄ¿Â¼ÏÂËÑË÷ÎÄ¼þ£¬
	  * Ä¿Â¼¿ÏÄÜ²»Ö¹Ò»¸ö£¬ËùÒÔÊ¹ÓÃÁ´±E
	  * Èç¹ûËùÓÐÄ¿Â¼ÏÂ¶¼Ã»ÓÐÕÒµ½¸ÃÎÄ¼þ£¬Ôò·µ»Ø0
	  */
	name = (char *)alloca(strlen(fname) + pfile->max_include_len + 2);
...
/* ´¦Àí°üÀ¨ÒýÓÃÍ·ÎÄ¼þ¡¢ÃüÁîÐÐºêºÍincludeÖ¸ÁîµÄ#inlcudeÀàµÄÖ¸Áî¡£
  * Èç¹û»º³åÑ¹Õ»³É¹¦Ôò·µ»ØÕæ¡£
  */
bool
...
enum include_type type;
{
	...
	else if (inc != NO_INCLUDE_PATH)
	{
		...
		/* Èç¹ûÊÇ#importÖ¸¶¨µÄÎÄ¼þ£¬ÔòÉèÖÃ²»ÄÜÖØ¸´¶ÁÈ¡ */
		if (type == IT_IMPORT)
			_cpp_never_reread(inc);
	}
...
#endif 
/* Studied not complain do not know how to use */
...
/* ÔÚ½«ÒýÓÃ»º³åµ¯³öÊäÈë¶ÑÕ»Ö®ºó½øÐÐÇåÀí¡£
  * Èç¹û»¹ÓÐÊ£ÓàµÄÒýÓÃÎÄ¼þ£¬½«ÆäÑ¹ÈëÕ»
  */
bool
...
struct include_file *inc;
{
	...
	/* ¼ÇÂ¼×èÖ¹ÒýÓÃºê£¬Èç¹û¸ÃºêÎªNULL Ôò±íÊ¾ÎÞ¿ØÖÆ */
	...
	inc->refcnt--;
	/* Èç¹ûÃ»ÓÐÕ»»º³åÊ¹ÓÃ¸ÃÎÄ¼þ£¬²¢ÇÒ¸ÃÎÄ¼þ±»ÉèÖÃÎª²»ÔÙ¶ÁÈ¡ */
	if (inc->refcnt == 0 && DO_NOT_REREAD(inc))
		/* ÊÍ·ÅÕâ¸öÎÄ¼þÕ¼ÓÃµÄ»º³å */
		purge_cache(inc);
...
/* ·µ»ØËÑË÷Ä¿Â¼
  * ¸ù¾ÝÃEûìÐ»òÕßÖ¸Áûí¡ÔñÄ¿Â¼»òÕßÑ¡ÔñÄ¬ÈÏÄ¿Â¼
  */
...
enum include_type type;
{
	...
	if (!buffer->search_cached)
	{
		...
		/* ÏÂÃæµÄÕâ¸öº¯ÊýÊÇlibiberty.libÖÐµÄº¯Êý*/
